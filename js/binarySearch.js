function foundValue(val, search) {
  return new Promise(resolve => {
    ...
    val.style.backgroundColor = "#FF00FF";
...
async function binarySearch() {
  ...
  while (lowIndex <= highIndex) {
    ...
    if (value1 == searchValue){
		searchValue = searchValue + ' : Found at index ' +midIndex; 
		searchContainer[0].childNodes[0].innerHTML = searchValue ;
  ...
  searchContainer[0].childNodes[0].innerHTML = searchValue + ' : Not Found';
  return null;
}